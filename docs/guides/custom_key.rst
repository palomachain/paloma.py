...
If none of the Key solutions provided by Paloma SDK or the community are able to meet your requirements,
...
Is the private key accessible by developer?
    * YES: Subclass :class:`paloma_sdk.key.raw.RawKey`
    * NO: Subclass :class:`paloma_sdk.key.key.Key`

Can the signing agent sign arbitrary data payloads AND use ECDSA on Secp256k1?
    * YES: Override :meth:`Key.sign()<paloma_sdk.key.key.Key.sign>`
    * NO: Override :meth:`Key.create_signature()<paloma_sdk.key.key.Key.create_signature>`
...
Can you determine the public key in advance, and is it static?
    ...
    * NO: Override :meth:`acc_address<paloma_sdk.key.key.Key.acc_address>`, :meth:`val_address<paloma_sdk.key.key.Key.val_address>`, :meth:`acc_pubkey<paloma_sdk.key.key.Key.acc_pubkey>`, :meth:`val_pubkey<paloma_sdk.key.key.Key.val_pubkey>` properties.
...
* External signing
    ...
    **Scenario:** The transaction signing is to be performed outside the Python program running Paloma SDK,
    such as signing via hardware wallet (Ledger, Trezor), etc. 


* Alternative signing algorithm

    **Scenario:** Paloma account you need to sign transactions for requires a signature algorithm other than
...
.. code-block:: python
    ...
    from paloma_sdk.key.raw import RawKey
